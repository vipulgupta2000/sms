...
$fname="";	$fval="";	$i=0;
	while($row = mysql_fetch_array($result1))
	{
		...
		$fval=$fval."'".mysql_real_escape_string(stripslashes(addslashes($_POST[$row['name'].$i])))."',";
...
$updateStatus =mysql_query("update load_master  set status='Validated' where id='$val'");
	...
	while($row = mysql_fetch_array($result1))
	{
		...
		$fname=$fname."'".mysql_real_escape_string(stripslashes(addslashes($_POST[$row['name'].$i])))."',";
                elseif($row['type']=='password')
