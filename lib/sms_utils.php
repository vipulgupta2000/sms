...
$mnth=$month+1;$yr=$year;if($mnth>12){$mnth=$mnth-12;$yr++;}
$dbdate1=setmydate('01'.'-'.$mnth.'-'.$yr);
$empid=$_POST['empid'.$i];
...
$projected=projected($empid,$off,$payr,$row['monthly_gross'],$tot);if($row['edate']<$dbdate1)$projected=$payr[$empid]['ytdgross']+$tot;
$b=$b.",".$projected;
//Savings declaration
...
if($row['edate']<$dbdate1){
$fname=$fname.",".'ytd_taxpaid';
$b=$b.",".$ytdtax;
$fname=$fname.",".'ytd_income';
$ytdincome=isset($payr[$empid]['ytdgross'])?$payr[$empid]['ytdgross']:0;$b=$b.",".$ytdincome;}
$fname=$fname.",".'incometax_month';$tax_month=round(($proj_tax-$ytdtax)/($off+1));
if($row['edate']<$dbdate1){$tax_month=round(($proj_tax-$ytdtax));echo "tax_month is".$tax_month;}
//New Code for Non Negative
...
$sql_qual_query_m = mysql_query("select month,year from paystate where status='Open'");
		$curr_month =mysql_result($sql_qual_query_m,0,0); 
		//$curr_year = date('Y');
		$curr_year = mysql_result($sql_qual_query_m,0,1); 
